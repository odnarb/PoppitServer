...
curl -w "\n" -H "X-Requested-With: XMLHttpRequest" -H "Content-Type: application/json" -X POST http://localhost:7777/user --data "{ \"first_name\": \"Test\",\"last_name\": \"Smith\",\"email_address\":\"test555@gmail.com\" }"

# PUT /user/:id route for editing a user
curl -w "\n" -H "X-Requested-With: XMLHttpRequest" -H "Content-Type: application/json" -X PUT http://localhost:7777/user/1 --data '{ "first_name": "Test","last_name": "Smith","email_address":"test555@gmail.com" }'
