module.exports = (opts) => {
    ...
    module.admin_registration_email = (user) => {
        ...
        html += `<strong>Group Name:</strong> ${user.group_name}<br />`
        if( parseInt(user.group_id) > 0 ){
            html += `<strong>Group Id:</strong> ${user.group_id}<br />`
        } else {
            html += "<strong>Group Id:</strong> <strong>ERROR: Group ID NOT FOUND</strong><br />"
        }
        html += `<strong>Group Country:</strong> ${user.country}<br />`
    ...
    module.group_leader_registration_email = (user) => {
        delete user.password_hash;

        let html = '<p>A new user has registered with your group!</p>'
        html += '<p>User information<br/>'
        html += `<strong>Group Name:</strong> ${user.group_name}<br />`
        html += `<strong>Group Country:</strong> ${user.country}<br />`
        html += `<strong>First Name:</strong> ${user.first_name}<br />`
